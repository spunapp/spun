...
function CreativeImage({ storageId }: { storageId: Id<"_storage"> }) {
  ...
  if (!url) return <div className="w-full h-20 bg-surface-alt rounded animate-pulse" />
      ...
      {message && (
        <div
          className={`flex items-center gap-1.5 text-[11px] ${
            state === "error" ? "text-red-600" : "text-spun"
          }`}
        >
...
function CreativePreviewCard({ creative }: { creative: Creative }) {
  return (
    <div className="rounded-md overflow-hidden border border-grid bg-white">
      {creative.imageStorageId ? (
        <CreativeImage storageId={creative.imageStorageId as Id<"_storage">} />
      ) : (
        <div className="bg-gradient-to-br from-spun to-spun-dark p-8 flex items-center justify-center min-h-[200px]">
      ...
      <div className="p-4 bg-white space-y-3">
        ...
        <p className="text-xs text-gray-600 leading-relaxed">{creative.copy}</p>
        <div className="flex items-center justify-between">
          <span className="px-3 py-1.5 bg-spun rounded-md text-xs font-medium text-white">
            {creative.cta}
          </span>
    ...
    <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {creatives.map((creative, i) => (
            <button
              ...
              className="group relative p-2 bg-white border border-grid rounded-md hover:border-spun/30 transition-all text-left overflow-hidden"
            >
              {creative.imageStorageId ? (
                <CreativeImage storageId={creative.imageStorageId as Id<"_storage">} />
              ) : (
                <div className="w-full h-16 rounded bg-gradient-to-br from-spun to-spun-dark flex items-center justify-center p-2">
                  <p className="text-[10px] font-semibold text-white/80 text-center leading-tight line-clamp-3">
                    {creative.headline}
                  </p>
                </div>
              )}
      ...
      {previewCreative && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="relative bg-white rounded-xl border border-grid max-w-md w-full max-h-[80vh] overflow-auto">
            <button
              onClick={() => setPreviewCreative(null)}
              className="absolute top-3 right-3 z-10 p-1 bg-gray-900 rounded-lg hover:bg-gray-800"
            >
